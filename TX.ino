...
void loop()
{
  ...
  bit_ppm[5][0] = data_avra &  0x00ff;
  bit_ppm[5][1] = (data_avra &  0xff00)>>8;
  
  data_avrb = map(analogRead(avrb),0,673,2000,1000);
  bit_ppm[6][0] = data_avrb &  0x00ff;
  bit_ppm[6][1] = (data_avrb &  0xff00)>>8;
...
void ppm_read(){
  ...
  if(ch_i>=0 && ch_i <= 4){
    bit_ppm[ch_i][0] = ch[ch_i] &  0x00ff;
    bit_ppm[ch_i][1] = (ch[ch_i] &  0xff00)>>8;
  }
  ch_i++;
}
